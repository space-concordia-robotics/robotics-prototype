...
def test_parse_command():
    ...
    """
    assert parse_command("estop") == ("estop", [])
    assert parse_command("arm_speed 2.25") == ("arm_speed", [2.25])
    assert parse_command("pid_constants 2 1.11 4.20 6.90") == ("pid_constants", [2, 1.11, 4.20, 6.90])
    assert parse_command("string_test 2 test 1.12 9.60") == ("string_test", [2, "test", 1.12, 9.60])
