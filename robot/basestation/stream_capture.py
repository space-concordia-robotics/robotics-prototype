...
def start_recording_feed(stream):
    ...
    return video_error_handling(stream, filename, save_video_dir, stream_url, recording_log_msg)

def video_error_handling(stream, filename, save_video_dir, stream_url, recording_log_msg):
    error_state = 0
    feed_ping = os.system('nc -vz ' + fetch_ros_master_ip() + ' 8080')
    if feed_ping == 0:
