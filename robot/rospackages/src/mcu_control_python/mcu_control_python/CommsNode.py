...
from robot.rospackages.src.mcu_control_python.mcu_control_python.commands.ArmCommands import (
    arm_out_commands,
    arm_in_commands,
)
from robot.rospackages.src.mcu_control_python.mcu_control_python.commands.WheelsCommands import (
    wheel_out_commands,
    wheel_in_commands,
)
from robot.rospackages.src.mcu_control_python.mcu_control_python.commands.ScienceCommands import (
    science_out_commands,
    science_in_commands,
)
...
from gps_publisher.gps_node import GpsPublisherNode
...
if len(sys.argv) > 0:
    ...
    local_selected_device = ROVER_SELECTED
else:
    local_mode = False
    ...
    if ser is not None and command is not None:
        ...
        for argument in zip(args, data_types):
            ...
            if data_type == dt.ARG_UINT8_ID:
                ...
                ser.write(
                    bytearray(struct.pack(">f", data))
                )  # This is likely correct now, will need to consult
...
class CommsNode(Node):
    def __init__(self):
        ...
        if local_mode:
            ...
            self.get_logger().info(
                "Science MCU is mounted at USB, attempting to open serial port"
            )
            start = time.time()
            ...
            while time.time() - start < 60:
                try:
                    ...
                    self.get_logger().warn(
                        "Retrying connection to science, error occured " + str(e)
                    )
                    time.sleep(1)
        else:
            ser = serial.Serial("/dev/ttyTHS2", 57600, timeout=1)
        ...
        self.get_logger().info(
            'Initialized "' + node_name + '" node for pub/sub/service functionality'
        )
        ...
        gps_pub_topic = "/gps_data"
        self.get_logger().info('Beginning to publish to "' + gps_pub_topic + '" topic')

        arm_command_topic = "/arm_command"
        self.get_logger().info(
            'Beginning to subscribe to "' + arm_command_topic + '" topic'
        )
        sub = self.create_subscription(
            String, arm_command_topic, self.arm_command_callback, 10
        )

        rover_command_topic = "/rover_command"
        self.get_logger().info(
            'Beginning to subscribe to "' + rover_command_topic + '" topic'
        )
        sub = self.create_subscription(
            String, rover_command_topic, self.rover_command_callback, 10
        )

        science_command_topic = "/science_command"
        self.get_logger().info(
            'Beginning to subscribe to "' + science_command_topic + '" topic'
        )
        sub = self.create_subscription(
            String, science_command_topic, self.science_command_callback, 10
        )

        rover_twist_topic = "/rover_cmd_vel"
        self.get_logger().info(
            'Beginning to subscribe to "' + rover_twist_topic + '" topic'
        )
        rover_twist_sub = self.create_subscription(
            Twist, rover_twist_topic, twist_rover_callback, 10
        )
    ...
    def arm_command_callback(self, message):
        self.get_logger().info(
            "received: " + message.data + " command, sending to arm Teensy"
        )
    ...
    def rover_command_callback(self, message):
        self.get_logger().info(
            "received: " + message.data + " command, sending to wheels Teensy"
        )
        command, args = parse_command(message)
    ...
    def science_command_callback(self, message):
        self.get_logger().info(
            "received: " + message.data + " command, sending to science Teensy"
        )
        command, args = parse_command(message)
