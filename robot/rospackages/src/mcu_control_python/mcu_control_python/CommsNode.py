...
if not local_mode:
    ...
    gpio.setmode(gpio.BOARD) # There are warnings about the carrier board being unoffical (kahboom dev kits are not offical!)
...
def main(args=None):
    if not local_mode:
        ser = serial.Serial('/dev/ttyTHS0', 57600, timeout=1)
    else:
    ...
    def __init__(self):
        ...
        if local_mode:
            ...
            while time.time() - start < 60:
                try:
                    ser_hardware = serial.Serial('/dev/ttyTHS0', 57600, timeout = 1)
                    ser = ser_hardware
            ...
            ser = serial.Serial('/dev/ttyTHS0', 57600, timeout = 1) # it is likely that the serial port is ttyTHS0 for UART #1: pins 8 for TX, 10 for RX
