...
def handle_send_gps(data):
    gpsData = struct.unpack("d" * 2, data)
    print("Received", list(map(lambda f: str(f), gpsData)))

    # To avoid re-instantiation of node every time
    if not hasattr(handle_send_gps, "gps_node"):
        handle_send_gps.gps_node = GpsPublisherNode(gps_pub_topic="/gps_data")
    handle_send_gps.gps_node.publish_gps_data(latitude=gpsData[0], longitude=gpsData[1])
...
wheel_out_commands = [
    ("set_motors", 0, [dt.ARG_UINT8]),
    ("motors_estop", 1, []),
    ("close_motors_loop", 2, []),
    ("open_motors_loop", 3, []),
    ("set_joystick", 4, [dt.ARG_UINT8]),
    ("set_gps", 5, [dt.ARG_UINT8]),
    ("set_enc", 6, [dt.ARG_UINT8]),
    ("set_acc", 7, [dt.ARG_UINT8]),
    ("get_rover_status", 8, []),
