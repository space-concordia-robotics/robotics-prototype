...
public:
    ...
    void setStatusMessage(const ubx_status_t & status){
        switch (status) {
            case UBLOX_STATUS_DATA_RECEIVED: sprintf(statusBuffer,"Data received.\n"); break;
            case UBLOX_STATUS_I2C_COMM_FAILURE : sprintf(statusBuffer,"I2C Comms error.\n"); break;
            case UBLOX_STATUS_CLASS_ID_MISMATCH: sprintf(statusBuffer,"Error in class and id matching. \n");  break;
            case UBLOX_STATUS_PREAMBLE_MISMATCH: sprintf(statusBuffer,"Error : Received wrong preamble. \n");  break;
            case UBLOX_STATUS_CRC_FAIL:  sprintf(statusBuffer,"Error : CRC verification failed. \n");  break;
...
//        }
     ...
     ubx_status_t pollNAV_SAT(char* res){
        ...
        for(int i = 0 ; i < numSatellites ; i++){
            ...
            switch (gnssID) {
                case 0: gnssType = const_cast<char*>("GPS"); break;
                case 1: gnssType = const_cast<char*>("SBAS"); break;
                case 2: gnssType = const_cast<char*>("Galileo"); break;
                case 3: gnssType = const_cast<char*>("BeiDou"); break;
                case 4: gnssType = const_cast<char*>("IMES"); break;
                case 5: gnssType = const_cast<char*>("QZSS"); break;
                case 6: gnssType = const_cast<char*>("GLONASS") ; break;
                default: gnssType = const_cast<char*>("Unknown") ; break;
            }
            switch (qualityIndex) {
                case 0: qualityInd = const_cast<char*>("no signal"); break;
                case 1: qualityInd = const_cast<char*>("searching signal"); break;
                case 2: qualityInd = const_cast<char*>("signal acquired"); break;
                case 3: qualityInd = const_cast<char*>("signal detected but unusable"); break;
                case 4: qualityInd = const_cast<char*>("code locked and time synchronized"); break;
                default: qualityInd = const_cast<char*>("code and carrier locked and time synchronized"); break;
            }
            if(qualityIndex >=4 ) {
                snprintf(res + strlen(res), RESULT_BUFFER_SIZE, "%s, %s, %u dBHz\n", gnssType, qualityInd,
                         signalStrength);
            }
            if(strlen(res) >= RESULT_BUFFER_SIZE ){return err;}
        }
        return err;
    }
    ...
    ubx_status_t receiveResponse(ubx_packet_t *outPacket,bool expectingAckOnly = false, ubx_packet_t* ackPacket = nullptr){
    ...
    uint16_t pollNAV_PVT(char* res){
        ...
        int n = snprintf(res,RESULT_BUFFER_SIZE - strlen(res),"NAV-HPPOSLLH : \n Latitude : %f , Longitude : %f , Height (MSL) : %f", navInformation.latitude.val / 10000000.f,
                         navInformation.longitude.val / 10000000.f,navInformation.height_msl.val / 1000.f);
        return strlen(res);
    }
    ...
    */
    ...
    {
    #ifdef _hwDebug
        snprintf(statusBuffer,RESULT_BUFFER_SIZE - strlen(statusBuffer),"Available bytes, got %i\n",bytesAvailable);
    #endif
        return UBLOX_STATUS_I2C_COMM_FAILURE;
    }
...
//            }
    ...
    ubx_status_t waitForResponse(ubx_packet_t *ubxPacket, ubx_packet_t * ackPacket = nullptr, bool expectingAckOnly = false) {
        ...
        if(status == UBLOX_STATUS_SUCCESS) {
            ...
            if(ackPacket != nullptr){
                ...
                ackPacket->header.payload_length = incomingDataBuffer[startPoint+4] | (incomingDataBuffer[startPoint+5] << 8);
