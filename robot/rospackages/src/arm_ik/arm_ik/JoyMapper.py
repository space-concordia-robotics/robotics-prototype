...
import rclpy
from robot.rospackages.build.arm_ik.arm_ik import JoyRemap
...
class JoyMapper(Node):
    ...
    def __init__(self):
        ...
        joy_mapper = JoyRemap()  # Start the joy_mapper node
        rclpy.spin_once(joy_mapper)  # Process one callback
        
        if joy_mapper.is_joy_received:
            print("No joystick data received. Exiting...")
            rclpy.shutdown()

        self.map_subscription = self.create_subscription(Joy, '/joy_remap', self.joy_callback, qos_profile)
    ...
    def joy_callback(self, report: Joy):
        ...
        # self.get_logger().info(f"Published data: {joy_data_str}")
