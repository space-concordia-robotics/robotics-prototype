...
# NOTE: Will have to re-add the code for GPIO
# uhg
        ...
        if ser.in_waiting > 0:
            ...
            if handler == None:
                print(
                    ...
                    " was found [for that received command]"
                )
                ser.read_until()  # 0A
...
def send_command(command_name, args, deviceToSendTo):
    ...
    if command is not None:
        ...
        print("Could not find command to send")
    return False
...
if __name__ == '__main__':
    #ser = serial.Serial('/dev/ttyS0', 57600, timeout = 1) original
    ser = serial.Serial('/dev/ttyACM0', 57600, timeout=1)  # on my dev machine
