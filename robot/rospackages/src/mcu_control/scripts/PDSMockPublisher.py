...
def PDS_pub():
    ...
    while not rospy.is_shutdown():
        #shift values
        voltage_current = shift_val(
            voltage_current, 'PDS_mock_mode_voltage', voltage_min, voltage_max, voltage_safe,
            voltage_speed
        )
        temperature1_current = shift_val(
            temperature1_current, 'PDS_mock_mode_temp1', temperature_min, temperature_max,
            temperature_safe, temperature_speed
        )
        temperature2_current = shift_val(
            temperature2_current, 'PDS_mock_mode_temp2', temperature_min, temperature_max,
            temperature_safe, temperature_speed
        )
        temperature3_current = shift_val(
            temperature3_current, 'PDS_mock_mode_temp3', temperature_min, temperature_max,
            temperature_safe, temperature_speed
        )

        #publish values
        pub_voltage.publish(voltage_current)
        pub_temperature.publish(
            Point(temperature1_current, temperature2_current, temperature3_current)
        )
