...
int nir_spectrum::createScanConfiguration(uint16 start, uint16 end){
    activeConfiguration.scanCfg.wavelength_start_nm = start;
    activeConfiguration.scanCfg.wavelength_end_nm = end;
...
void nir_spectrum::close()
{
    USB_Close();
    USB_Exit();
}

int nir_spectrum::scan()
{
    ...
    if(runScan(pDataBlob,&fileSize) != PASS){
        ...
        cout << "Successfully ran scan" << endl;
    }
...
void nir_spectrum::SaveToCSV(string filename,std::vector<std::pair<std::string, std::vector<double>>> dataset)
{
...
vector<double> nir_spectrum::GetWavelengths()
/**
...
vector<double> nir_spectrum::GetIntensities()
/**
...
vector<double> nir_spectrum::GetRefIntensities()
/**
...
vector<double> nir_spectrum::GetAbsorbance()
/**
...
//get reflectance
vector<double> nir_spectrum::GetReflectance()
/**
 * This function is for accessing the absorbance vector from data last
 * set using SetData()
 */
...
int nir_spectrum::getAllConfigurations(void)
{
...
void nir_spectrum::printCurrentConfiguration(){
...
void nir_spectrum::printAllConfigurations() {
...
int nir_spectrum::runScan(void *pData, int *pBytesRead)
{
    int size;
...
void *nir_spectrum::GetRefCalMatrixBlob()
{
...
void *nir_spectrum::GetRefCalDataBlob()
{
    return refCalDataBlob;
}

int nir_spectrum::FetchRefCalData()
{
...
int nir_spectrum::FetchRefCalMatrix()
{
...
int nir_spectrum::SetData(void* pScanDataBlob, void *pRefDataBlob)
/**
