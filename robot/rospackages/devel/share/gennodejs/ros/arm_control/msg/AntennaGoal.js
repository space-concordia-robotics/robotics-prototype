// Auto-generated. Do not edit!

// (in-package arm_control.msg)


"use strict";

const _serializer = _ros_msg_utils.Serialize;
const _arraySerializer = _serializer.Array;
const _deserializer = _ros_msg_utils.Deserialize;
const _arrayDeserializer = _deserializer.Array;
const _finder = _ros_msg_utils.Find;
const _getByteLength = _ros_msg_utils.getByteLength;

//-----------------------------------------------------------

class AntennaGoal {
  constructor(initObj={}) {
    if (initObj === null) {
      // initObj === null is a special case for deserialization where we don't initialize fields
      this.desiredDir = null;
      this.distFromBase = null;
    }
    else {
      if (initObj.hasOwnProperty('desiredDir')) {
        this.desiredDir = initObj.desiredDir
      }
      else {
        this.desiredDir = 0.0;
      }
      if (initObj.hasOwnProperty('distFromBase')) {
        this.distFromBase = initObj.distFromBase
      }
      else {
        this.distFromBase = 0.0;
      }
    }
  }

  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type AntennaGoal
    // Serialize message field [desiredDir]
    bufferOffset = _serializer.float64(obj.desiredDir, buffer, bufferOffset);
    // Serialize message field [distFromBase]
    bufferOffset = _serializer.float64(obj.distFromBase, buffer, bufferOffset);
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type AntennaGoal
    let len;
    let data = new AntennaGoal(null);
    // Deserialize message field [desiredDir]
    data.desiredDir = _deserializer.float64(buffer, bufferOffset);
    // Deserialize message field [distFromBase]
    data.distFromBase = _deserializer.float64(buffer, bufferOffset);
    return data;
  }

  static getMessageSize(object) {
    return 16;
  }

  static datatype() {
    // Returns string type for a message object
    return 'arm_control/AntennaGoal';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return '3e3ff4c1f14859f1d5d7315b6ab697b9';
  }

  static messageDefinition() {
    // Returns full string definition for message
    return `
    float64 desiredDir
    float64 distFromBase
    
    `;
  }

  static Resolve(msg) {
    // deep-construct a valid message object instance of whatever was passed in
    if (typeof msg !== 'object' || msg === null) {
      msg = {};
    }
    const resolved = new AntennaGoal(null);
    if (msg.desiredDir !== undefined) {
      resolved.desiredDir = msg.desiredDir;
    }
    else {
      resolved.desiredDir = 0.0
    }

    if (msg.distFromBase !== undefined) {
      resolved.distFromBase = msg.distFromBase;
    }
    else {
      resolved.distFromBase = 0.0
    }

    return resolved;
    }
};

module.exports = AntennaGoal;
