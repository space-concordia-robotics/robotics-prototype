...
class DCMotor : public RoverMotor {
  public:
    ...
    // for new driver
    DCMotor(int dirPin, int pwmPin, float gearRatio); // if no encoder
    // for new driver
    DCMotor(int dirPin, int pwmPin, int encA, int encB, // if all data is known
...
// for new driver
DCMotor::DCMotor(int dirPin, int pwmPin, int encA, int encB, // if all data is known
                 uint32_t port, int shift, int encRes, float gearRatio):
  directionPin(dirPin), pwmPin(pwmPin)
{
  numDCMotors++;
