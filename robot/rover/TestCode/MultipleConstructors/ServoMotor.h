...
class ServoMotor : public RoverMotor {
  public:
    ...
    ServoMotor(int pwmPin, float gearRatio); // if no encoder
    ServoMotor(int pwmPin, int encA, int encB, // if all data is known
...
ServoMotor::ServoMotor(int pwmPin, int encA, int encB, // if all data is known
                       uint32_t port, int shift, int encRes, float gearRatio):
  pwmPin(pwmPin)
{
  numServoMotors++;
