#ifndef ENCDATA
#define ENCDATA

#include <cstdint>
#include "SPI.h"

#define CRC_DEFAULT_VALUE 0xFF
#define CS_PIN            16
#define CLOCK_RATE        10000

#define REG_STATUS    (0x00)
#define REG_ACSTAT    (0x01)
#define REG_ANGLE_VAL (0x02)
#define REG_MOD1      (0x06)


const uint8_t CRC_Table[256]
        {
                
        };
const SPISettings TLE5012B_SPI_SETTINGS(CLOCK_RATE, MSBFIRST, SPI_MODE1);

struct Encoder {
    uint8_t address;
    union angle_union {
        float _float;
        byte _bytes[sizeof(float)];
        uint16_t _raw;
    } angle;
    int status;
    int crc_fails;
    int address_fails;
    int total_transactions;
    char* status_msg;
};

enum StatusCodes{
    OK = 0 ,
    ADDRESS_FAIL = -1,
    CRC_FAIL = -2,
    NON_FUNCTIONAL = -3
};

/**
 *
 * @param address : Encoder address, either 0x90,0xA0,0xB0
 * @param reg     : Register to read from
 * @param data    : 16 bit word to write
 * @return        : Status of transaction
 */
int EncoderWrite16(uint8_t address, uint8_t reg, uint16_t val);

/**
 *
 * @param address : Encoder address, either 0x90,0xA0,0xB0
 * @param reg     : Register to read from
 * @param data    : 16 bit word to write
 * @param size
 * @return        : Status of transaction
 */
int EncoderRead16(uint8_t address, uint8_t reg, uint16_t *data, uint8_t size);

/**
 *
 * @param txAddress : Address to be sent to the device (such as 0x90)
 * @param rxAddress : Acknowledged address from device (such as 0x99)
 * @return          : Status of check
 */
int checkAddress(uint8_t txAddress, uint8_t rxAddress);

/**
 *
 * @param message : 4 bytes, 2 bytes for transmitted command and 2 for either received or transmitted data.
 * @param length  : Length of message, usually 4.
 * @return
 */
static inline uint8_t CRC8(const uint8_t *message, uint8_t length) {
  uint8_t crc = CRC_DEFAULT_VALUE;

  for (int i = 0; i < length; i++) {
    crc = CRC_Table[crc ^ *(message + i)];
  }
  return (~crc);
}

/**
 *
 * @param encoder : Struct that contains the info for an encoder.
 */
inline void updateState(struct Encoder& encoder) {

  if( (encoder.crc_fails + encoder.address_fails) > (0.2 * encoder.total_transactions)){
    strncpy(encoder.status_msg,"Encoder non functional", 100);
    encoder.status = NON_FUNCTIONAL;
  }
  else if(encoder.status == ADDRESS_FAIL){
    strncpy(encoder.status_msg,"Addressing with encoder slave failed", 100);
    encoder.address_fails++;
  }
  else if(encoder.status == CRC_FAIL){
    strncpy(encoder.status_msg,"CRC check failed", 100);
    encoder.crc_fails++;
  }
  encoder.total_transactions++;
}


#endif