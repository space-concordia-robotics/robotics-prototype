  ...
  if (budgeDir <= 1 && budgeSpeed <= MAX_SPEED && budgeTime <= MAX_BUDGE_TIME && budgeTime >= MIN_BUDGE_TIME) {
    ...
    switch (motorType) {
      ...
      case SERVO_MOTOR:
        ...
        sinceStart = 0;
        Serial.print("sinceStart before loop: ");Serial.println(sinceStart);
        if (budgeDir == CLOCKWISE && canTurnRight) {
          Serial.println(cwSpeed);
          Serial.println(pwmPin);
          analogWrite(pwmPin, cwSpeed);
          Serial.println("cw between");
          while (sinceStart < budgeTime) ; // wait
          Serial.println("cw if 2");
          analogWrite(pwmPin, PWM_STOP); // sets duty cycle to 50% which corresponds to 0 speed
        }
        if (budgeDir == COUNTER_CLOCKWISE && canTurnLeft) {
          Serial.println("ccw if");
          analogWrite(pwmPin, ccwSpeed);
          while (sinceStart < budgeTime) ; // wait
          Serial.println("ccw if 2");
          analogWrite(pwmPin, PWM_STOP); // sets duty cycle to 50% which corresponds to 0 speed
        }
        ...
        Serial.print("sincestart: ");Serial.println("sinceStart");
        Serial.print("left? ");Serial.println(canTurnLeft);
        Serial.print("right? ");Serial.println(canTurnRight);
        Serial.print("dir: ");Serial.println(budgeDir);
