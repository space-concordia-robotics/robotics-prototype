...
void loop() {
  if (millis() - prevRead > 200)
  {
    ...
    if (Serial.available() > 0)
    {
      ...
      desiredVelocityRight = Serial.parseFloat() ;
      if (desiredVelocityRight == -2 ) {
        digitalWrite(TEST_DIR, LOW);

      }
      else if (desiredVelocityRight == -1 )
      {
      ...
      else
      {
        ...
        analogWrite(TEST_PWM, desiredVelocityRight);
        //analogWrite(DC_PIN_FRONT_RIGHT, velocityRight.toInt());
        Serial.print("Throttle: "); Serial.print(desiredVelocityRight);
    ...
    else
    {
      ...
      if (steering < 0 )
      {
        ...
        desiredVelocityRight = mapFloat(throttle * deg, minThrottleController, maxThrottleController, 0, maxVelocity);
        desiredVelocityLeft = mapFloat((-1) * throttle, minThrottleController, maxThrottleController,  0, maxVelocity);
      ...
      if (steering >= 0 )
      {
        ...
        desiredVelocityRight = mapFloat(throttle, minThrottleController, maxThrottleController, 0, maxVelocity);
        desiredVelocityLeft = mapFloat((-1) * throttle * deg, minThrottleController, maxThrottleController, 0, maxVelocity);
...
void dcInterrupt(void) {
  RF.setVelocity(RF.rotationDirection, RF.getDesiredVelocity(), RF.getCurrentVelocity());
